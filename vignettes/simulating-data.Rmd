...
```{r}
df %>% sim_discr(p = 5, var = 1, cov = 0.1, group = "Y", group_means = c(1, -1))
```


# Chaining functions with `%>%`

One of the advantage of the `tidymvsim` package is the ability to chain functions together to create complex data covariance structures. You can change functions together in any order although the `sim_discr()` function requires a grouping variable.
...
df <-
  ...
  sim_covar(p = 5, var = 1, cov = 0.7, name = "high")            #5 variables with high covariance
df
```
...
df1 <-
  ...
  sim_discr(p = 5, var = 1, cov = 0.1, group = "factor", group_means = c(-1, 1), name = "discr") %>% 
  sim_discr(p = 5, var = 0.5, cov = 0.2, group = "factor", group_means = c(0, -0.5), name = "discr2")
df1
```
...
x <-
  ...
  rearrange() %>% #arrages by strength of correlation
  shave() #removes uppper triangle

fashion(x) #pretty printing of correlation dataframes
```
...
```{r}
# chemhelper::plot_pca(pca, group_var = factor)
```
...
```{r}
plsda <- opls(select(df2, -factor), df2$factor, plotL = FALSE, printL = FALSE)
```
```{r}
