...
And we can use some accessor functions to get scores, loadings, and VIP scores (for PLS) as tibbles.
...
What's the effect of missing data on a statistical analysis? The `sim_missing()` function replaces a proportion of values with NA.  Let's see how it affects a PLS-DA analysis.
...
plsda.q2 <- 
  map_dbl(dfs,
      ~my_opls(select(., -factor), .$factor) %>%
        get_modelinfo() %>% .$model_stats %>% .$`Q2(cum)`)
...
It looks like, on average in this sample, the imputation by `mice` did a better job, at least with respect to the $Q^2$ statistic.
